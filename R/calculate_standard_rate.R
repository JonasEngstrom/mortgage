...
#'
#' Calculate the standard rate of capital income, using [the method descirbed by
...
#' }
calculate_standard_rate <- function(government_borrowing_rate_table = gbr_table) {
  NULL ->
    gbr_table ->
    gbr ->
    standard_rate
  ...
  government_borrowing_rate_table |>
    ...
    dplyr::mutate(standard_rate = dplyr::if_else(standard_rate < 1.25, 1.25, standard_rate)) |>
